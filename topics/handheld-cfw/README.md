...
### Knulli と ROCKNIX の選び方（2026-09-25）

見た目はどちらも EmulationStation で、PortMaster もある。分かれるのはカーネルと、ポケットに入れるときの電源の扱い。

| 基準 | Knulli | ROCKNIX |
|------|--------|---------|
| 中身 | 出荷に近い Allwinner BSP 4.9 と Mali | Mainline と Panfrost。設定上 1.5 GHz |
| 電源ボタン | 短押しのサスペンドを公式機能として記載 | Fake suspend。H700 ではスリープ不能という短期利用の報告がある |
| HDMI | 公式機能 | ケーブル接続中は Fake suspend しない |
| RetroArch | ES が設定を戻すことがある | ホットキー自動設定を切れば、自分の設定が残る |
| 更新 | 本体から OTA | イメージの差し替え |
| Wi-Fi | H700 は WPA2+WPA3 混在が非対応。WPA2 にする | 公式ページは Network Settings から有効化 |
| 導入 | 機種名のイメージを焼く | H700 イメージに加え、パネル改訂で DTB が 2 種。ゴミ表示ならもう一方 |

- **ポケットに入れて、HDMI や Bluetooth、箱絵を使う:** Knulli。手元の Syncthing では、セーブとステートが同じフォルダになりやすいので、機種ごとにサブフォルダを切る。
- **コアの設定を自分で持ち、Mainline に寄せる:** ROCKNIX。画面がゴミなら `sun50i-h700-anbernic-rg35xx-h.dtb` ともう一方の rev6 を入れ替える。スリープは期待しない。

起動の短さとメニューの軽さだけが基準なら、この二者の外側に muOS がある。
